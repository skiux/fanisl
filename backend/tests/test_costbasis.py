...
def test_manual_avg_cost_prices_the_whole_position():
    """重放只见过 1 个 BNB，实际持有 6.712 个——录了均价就整仓按它算。

    这正是"划转 / 派息 / 小额兑换进来的币"那一类：myTrades 里没有痕迹，
    重放报 unpriced_qty，未实现比实际小一截。
    """
    lots = replay([trade("BNBUSDT", buy=True, qty=1.0, price=650.0, at=DAY)])
    held = {"BNB": 6.712}

    bare = summarize(lots, {"BNB": 682.15}, held=held)
    assert bare["assets"][0]["unpriced_qty"] == pytest.approx(5.712)
    assert bare["unrealized_usd"] == pytest.approx(32.15)

    fixed = summarize(lots, {"BNB": 682.15}, held=held, overrides={"BNB": 600.0})
    row = fixed["assets"][0]
    assert row["unpriced_qty"] == 0.0
    assert row["avg_cost_usd"] == 600.0
    assert row["cost_source"] == "manual"
    assert row["cost_known"] is True
    assert fixed["unrealized_usd"] == pytest.approx(6.712 * (682.15 - 600.0))


def test_manual_avg_cost_does_not_touch_realized():
    """当前的均价回答不了过去那些卖出是赚是赔，按它重算已实现就是编的。"""
    lots = replay([
        trade("BNBUSDT", buy=True, qty=2.0, price=500.0, at=DAY),
        trade("BNBUSDT", buy=False, qty=1.0, price=700.0, at=2 * DAY),
    ])
    plain = summarize(lots, {"BNB": 700.0}, held={"BNB": 1.0})
    fixed = summarize(lots, {"BNB": 700.0}, held={"BNB": 1.0}, overrides={"BNB": 300.0})
    assert fixed["realized_usd"] == pytest.approx(plain["realized_usd"])
    assert fixed["realized_usd"] == pytest.approx(200.0)


def test_asset_held_only_in_futures_wallet_still_shows_up():
    """整仓划进合约、从没在现货买过的币——重放里没有它，但账户里实实在在有。

    只从 lots 出发的话它整行消失，界面上既看不见持仓也没有地方给它录均价。
    """
    out = summarize({}, {"BNB": 682.15}, held={"BNB": 4.0})
    row = next(r for r in out["assets"] if r["asset"] == "BNB")
    assert row["qty"] == pytest.approx(4.0)
    assert row["unpriced_qty"] == pytest.approx(4.0)
    assert row["avg_cost_usd"] is None and row["unrealized_usd"] is None
    assert row["cost_source"] is None
    assert out["unrealized_usd"] == 0.0


def test_manual_avg_cost_works_for_never_traded_asset():
    out = summarize({}, {"BNB": 682.15}, held={"BNB": 4.0}, overrides={"BNB": 600.0})
    row = next(r for r in out["assets"] if r["asset"] == "BNB")
    assert row["unrealized_usd"] == pytest.approx(4.0 * 82.15)
    assert row["cost_source"] == "manual"


def test_cost_source_says_where_the_average_came_from():
    lots = replay([trade("BNBUSDT", buy=True, qty=2.0, price=500.0, at=DAY)])
    rows = {r["asset"]: r for r in
            summarize(lots, {"BNB": 700.0, "USDT": 1.0}, held={"BNB": 2.0, "USDT": 50.0})["assets"]}
    assert rows["BNB"]["cost_source"] == "trades"
    assert rows["USDT"]["cost_source"] == "cash"
