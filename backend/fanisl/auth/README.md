...
```bash
# 在服务器 backend/ 目录下
.venv/bin/python -m fanisl.auth.bootstrap alice
```
...
另有 `tests/test_startup.py`：**起独立进程**验 `fanisl.main` 能 import
（同进程 import 一次就缓存了，验不出东西）。runtime 在模块级就建连接池与客户端，
构造期的任何异常都等于全站 502，而 nginx 只会给一句 Bad Gateway。

```bash
PYTHONPATH=. .venv/bin/python -m pytest tests/test_auth.py tests/test_startup.py -q
```
