...
1. `python -m fanisl.knowledge.nodes export` → 导出待归并单元清单（method/concept
...
3. 产出 nodes JSON（格式见 §5）→ `python -m fanisl.knowledge.nodes import <file>`
...
4. `python -m fanisl.knowledge.nodes recompute` → 重算全部节点状态（幂等，任何时候可跑；
   评分入库后应例行跑，已挂入每日流程）。
...
归并解决"同一命题"，关系边表达**不同命题之间**的结构（表 `node_relations`，人工判，
JSON 导入同 §5 模式，`nodes.py import-relations`）：

- `conflicts`：**对立命题**——对同一对象给出不相容的结论（例：半导体"数字地租、周期已破"
  vs "周期性涨法仍在"）。这是发现层最稀缺的信号，判定标准从严：两命题不能同真，
  且各自有独立论证；仅角度不同/侧重不同不算。note 必填对立点。
- `relates`：**高置信互补**——问题↔解法、识别↔应对、同主题跨源互补视角。判定标准：
  读其一时明显应看另一（导航价值），泛泛的主题相似不建边。宁缺勿滥。

方向：边是无向的，(a,b) 与 (b,a) 视为同一条（导入时按 id 排序归一）。共识不建边——
跨源共识的载体是节点内的跨源提及（`n_creators≥2`，API 以 cross_source 过滤）。
