...
- `extraction-guide.md` — L1 提取规范 **v3**（冻结；判断规则 + 期限映射 + 幅度词表 + 标签受控词表；v1→v2、v2→v3 改动见其 §11、§12）
...
```
python -m fanisl.knowledge.daily      # 行情刷新 → 到期评分 → 节点状态重算 → 补齐缺帧（幂等）
```
...
```
python -m fanisl.knowledge.backfill_transcripts @yttalkjun --since-days 4   # 只拉某个信源
