...
def browse_units_page(
    ...
    *,
    ...
    conditions = ["c.status <> 'superseded'", ACTIVE_RUN]
    params: list[object] = []
    if kind:
...
def verification_summary(pool: ConnectionPool, *, days: int) -> dict[str, Any]:
    ...
    with pool.connection() as conn:
        ...
        due = conn.execute(r"""
            ...
            WHERE u.kind='claim' AND c.status <> 'superseded' AND """ + ACTIVE_RUN + r"""
    ...
    with pool.connection() as conn:
        if bucket == "due":
            ...
            common = f"""
                ...
                JOIN creators cr ON cr.id=u.creator_id
                JOIN contents c ON c.id=u.content_id
                WHERE s.outcome = ANY(%s) AND c.status <> 'superseded' AND {ACTIVE_RUN}
            """
