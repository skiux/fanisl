...
用法：python -m fanisl.knowledge.prices [--since 2026-04-15]（幂等 upsert）
"""
...
# 采集口径来自 `fanisl.assets` 登记表——**加/减符号改那里，不要在这里再列一份**。
# 这两个名字保持原样导出，是因为 scorers/estimates 与 CLI 都按它们取值。
# 已核不可用（勿反复试）：恒生科技指数 —— ^HSTECH 无数据，HSTECH.HK 一个月仅 1 根有效，
# 3033.HK（ETF）价位口径与指数点位不同、无法验"突破 4850"类判断 → 恒科 claim 只能 priceable=false。
# 2026-08-14 复核补充：3067.HK 有连续日线（近 4 个月 74 根，9.0~10.9 港元），只是同样不是点位
# 口径。所以"点位型"恒科判断仍然无解，但将来若出现"恒科跑赢/跑输"这类方向或相对判断，
# 可用 3067.HK 评——届时再单独登记，不要因为它能跑就把点位型 claim 也标成 priceable。
SYMBOL_MAP: dict[str, tuple[str, float, str]] = assets.yf_symbol_map()
FRED_SERIES: dict[str, str] = assets.fred_series()
