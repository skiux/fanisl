...
用法：
  python -m fanisl.knowledge.nodes export            # 导出待归并单元清单
  python -m fanisl.knowledge.nodes import <file>     # 导入归并 JSON（整文件事务）
  python -m fanisl.knowledge.nodes recompute         # 重算全部节点状态
  python -m fanisl.knowledge.nodes retire <id> <理由> # 人工退役节点
"""
    ...
    try:
        ...
        if cmd == "export":
            ...
            if "--commit" not in sys.argv:
                ...
                print("\n确认无可归并项后加 --commit 落库：")
                print(f"  python -m fanisl.knowledge.nodes seed-singletons {version} --commit")
