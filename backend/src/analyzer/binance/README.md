...
```
unrealized.spot_usd     成交重放出加权平均成本 → (市价 − 均价) × 成本已知的数量
...
carry.*                 资金费 / 手续费 / 返佣
daily[]                 每天落袋多少：income 逐行按天分桶 + 现货成交按天结转
```
...
**① 均价乘错了数量（现货未实现被放大六倍多）。** `summarize` 里 `avg_cost` 是
`cost_usd / 重放数量`，而 `qty` 用的是真实余额。成交历史只覆盖**能猜到交易对**的币，
划转 / 理财派息 / 小额兑换进来的从不出现在 `myTrades` 里——代码却假设它们和见过的
同价。实测重放 1 个 BNB @ $650、实际持有 6.712 个，未实现算成 +$215.79，
有据可依的只有 +$32.15。

现在只对 `min(余额, 重放数量)` 算，多出来的报在 `unpriced_qty`。数量仍然按余额，
那是账户里真有的。**别在客户端拿 `qty × (price − avg_cost)` 重算**。
...
`/sapi/v1/margin/account` 的 `userAssets` 原先整段没读。成本基础按"账户一共有多少"
...
### `unpriced_qty` 剩下的那部分，由人手补

上面 ① 修完之后，"成本不明"的数量是**如实报出来了**，但它仍然不计入盈亏——
划转 / 派息 / 小额兑换 / 闪兑进来的币在 `myTrades` 里没有任何痕迹，
`capital/deposit/hisrec` 又只回 90 天，更早的充值永远查不回来。
**缺的是历史，不是算法**：再改重放也变不出这些成本。

所以开了一条人工通道：`costbasis_store.py` + `/admin/cost-basis`（仅管理员），
按币录一个持仓均价，`summarize(overrides=...)` 收下它。规则：

- 录了就**整仓**按它算，`unpriced_qty` 归零，`cost_source` 报 `manual`；
- **只影响未实现**。当前的均价回答不了过去那些卖出是赚是赔，照它重算已实现就是编的；
- `summarize` 遍历 **余额 ∪ 重放 ∪ 录入**。只从 `lots` 出发的话，整仓划进合约、
  从没在现货买过的币压根不在列表里——而它恰恰最需要人手填。

**还能往前走一步（没做）：** `convert_trade_flow`（闪兑）、`dust_log`（小额兑换）、
`earn_*_rewards`（派息）这三条路是有接口的，`ledger.py` 已经在拉了，而 `replay()`
本来就收 `rewards=`（`portfolio.py` 现在传的是空列表）。喂进去能把最近 30 天的
"成本不明"消掉一部分。没做的原因是：这三个端点都是 30 天窗口，补不上更早的历史，
而资产页每次刷新要多打三四个来源。真要做，先量一下权重预算。
