 ...
 */
export type PageKey = 'assets' | 'orders' | 'ledger' | 'account' | 'admin' | 'costbasis'
...
const EXTRA_TITLES: Partial<Record<PageKey, string>> = {
  account: '账号', admin: '用户管理', costbasis: '现货成本',
}
...
const PAGE_KEYS: PageKey[] = [...PAGES.map((page) => page.key), 'account', 'admin', 'costbasis']
 ...
 * 再看见一屏错误——那把权限问题讲成了故障。
 */
const ADMIN_ONLY: PageKey[] = ['admin', 'costbasis']

export function canView(page: PageKey, role: 'admin' | 'member') {
  return !ADMIN_ONLY.includes(page) || role === 'admin'
}
