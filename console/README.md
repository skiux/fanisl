...
PC 上是**定高工作台**，不是长滚动页。两列结构：

- 左列上排 净值 + 30 天曲线，下方工作区独占剩余高度，明细收进标签页
  （归因 / 现货 / 理财 / 合约仓位）
- 右列跨两行：风险 → 钱包分布 → 理财，取数来源钉在底部

首行按内容定高（`minmax(13rem,auto)`），**多余高度全部给工作区**——那里的
瀑布图与表格用得上，其余地方按比例分只会把空洞挪个位置。窄屏退回单列滚动，
并用 `order` 把工作区提到概览栏之前。
