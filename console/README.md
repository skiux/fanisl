...
左栏六个入口各自带着本节的头条数字——不点也能一眼看全六个答案；点进去
右边才展开那一节的完整明细。摘要留在导航列，明细一次只占一块区域，
页面因此既不堆、也不需要深滚。
...
├───────────────┬──────────────────────────────┤
│ 一 总览  $80,141.23                          │
│ 二 本期变动 +$2,960.81 │   选中的那一节        │
│ 三 现货  $49,122.30    │   （区域内部滚动）     │
│ 四 理财  $14,958.52    │                      │
│ 五 合约    +$380.45    │                      │
│ 六 风险      17.5%     │                      │
│ ─────────              │                      │
...
- 桌面（≥1024）把整张纸钉在视口高度内，明细区自己滚：**切换分节时页面高度
  波动为 0**，不跳版也不需要深滚。窄屏改为堆叠，导航在上、明细在下。
- 降级在摘要列里就看得见：合约取不到时导航直接显示「— / 取不到」，
  不用点进去才发现。
- **不靠删信息降密度。** 现货的价格列、钱包的百分比都保留——一次只显示一节，
  空间不再紧张，该有的列就该有。
