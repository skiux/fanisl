...
内容本来就是报表结构：期初 → 各项变动 → 期末是一张对账表，持仓是明细表，
风险是附注。报表天然有层级（报头/正文/附注是不同物种），也天然允许留白——
页面有页边距，空白是版式的一部分而不是漏洞。

- 纸张放在桌面上（`.sheet` / `--desk`），超宽屏下版心不被拉散
- 报头 → 一条实心重线（整份唯一）→ 摘要 → 正文两栏 → 口径附注
- 六节按序号编号，左栏 一 本期变动 / 二 现货持仓 / 三 理财持仓，
  右栏 四 资产分布 / 五 合约仓位 / 六 风险
- **明细平铺，不用标签页。** 文件的长度不该因为你点了什么而变化；标签页是
  app 的做法，塞进文件概念里就会出现切换时 260px 的跳动
- 本期变动用**表**而不是图：能逐行核对精确读数，右侧配以零为轴的发散条比量级
- 两栏排版在 `xl`(1280) 才展开——1024 下左栏约 530px、右栏约 380px，
  价格列会被裁、钱包数值会顶破容器
- **浅色为主设计目标**，深色是同一份报表在暗处的读法

三种字体各有职责：标题用 Newsreader（衬线），标签用 Geist，数字一律 Geist Mono
（tabular）。一套字体走天下最省事，也最没有表情。
