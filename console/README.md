...
- **每节先给一个头条数字**（27px），行退成 12.5px 的支撑材料——眼睛先落在六个
  大数上，再决定读不读明细。这是密度问题的主要解法：建立阅读路径，而不是删信息
- **节内行间不画线**，用行距分组；只保留节界、表头与合计行。分隔线从 35 条降到
  14 条——这是感知密度下降最快的一招，且一个数据都不动
- **同一信息不说两遍**：现货去掉「价格」列（数量 × 价格 = 价值，三取二即可），
  资产分布去掉百分比数字（条已经表达比例）。头条已给出的数字，节内不再复述
