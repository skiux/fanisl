...
| K3 | **提取+沉淀**：提取规范冻结（四类知识平权、期限映射、标签体系，见 `knowledge/extraction-guide.md`）→ import 管线 → 试提取 2 条人审 → 18 条全提取，知识库 v0 可浏览可筛 | 积累、检索 | 🔄 2026-07-16 |
| K4 | **验证**：日线价格源（美股/ETF/指数缺口）+ 评分器×5 + 每日到期任务 + 随机基线 + 验证结果写回单元；联赛表作为视图上线 | 可验证 | ⬜ |
| K5 | **归并与检索**：规范知识节点表 + 提及记录表 + 对真实重复的首次归并 + 生命周期状态 + 前端按主题浏览（归并判据对着真实重复样本设计，故后置于 K3） | 复用、学习 | ⬜ |
