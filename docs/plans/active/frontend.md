...
- **卡片减负与全站样式统一**（2026-09-23，用户：卡片信息太密、样式一般；多个页面颜色样式不统一）。已提交为 `2628888`，2026-09-24 随 knowledge 席位的推送（`6c603f4`）一起上线：
  ...
  - 真实数据只看到了档案页（隧道 09-24 中途又断；知识库那次截到的是后端不通时的仓库离线样本），其余页面用 e2e 夹具看过，
    隧道恢复后要用真实数据把五页补看一遍
...
## Next
- 首页（空间叙事）左边距按 7vw 给，顶栏加宽到 1360 后与顶栏左缘不再对齐（1440 下差 64px）；首页是整页艺术化布局，未动
...
- **base**：nginx 开 gzip（见上「加载慢的排查」与 `base.md`），验证页首屏的主要瓶颈
...
## Requests in
- **knowledge 席位（2026-09-24，提取规范 v3）**：① claim 载荷新增 `grade_note`（定级说明：为什么判这一级、
  代理与口径怎么定），请在单元详情与判定档案里「标的说明」之后显示为「定级说明」，没有就不显示。v3 起理由
  不再写进 `asset_text`，所以不显示它就看不到 D 级的理由；v2 存量单元没有这个字段，理由仍在 `asset_text` 里。
  ② `magnitude` 的 `target / low / high` 可以是按阶梯日给值的对象 `{"YYYY-MM-DD": 数字}`（幅度词按期限分档时用，
  例 #1596）。现在的 `asNumber` 会把它静默跳过、不报错，但阈值也就不显示了；判定档案与价格证据图最好按
  阶梯日取对应的值。契约改动已请 base 更新 `api.md` §5.0
已处理（2026-09-14）：
