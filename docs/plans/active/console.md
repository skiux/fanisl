...
- ~~逐日盈亏缺理财派息~~ —— 2026-09-25 由 knowledge 席位按用户要求修（`client.earn_rewards_history`：90 天窗按
  ≤30 天切三段、每段翻页，取不全整体失败；`type=ALL` 此前已改）。测试替身按请求时间窗过滤派息行，与真接口一致。上线后生产实测：活期 90 天 209 条，定期 0 条，不再报 -6021。
  仍未做：flows.* 不进「取数状态」，这类来源失败时界面上看不出来；流水页 30 天窗的派息只取第一页（每页 100 条），
  活期按币按日派息：2026-09-25 生产实测 90 天 209 条（BNB、USDC、USDT），约每 30 天 70 条，已接近单页上限，
  再多一个派息币就会被截断。修法同逐日盈亏，改用 `client.earn_rewards_history`
