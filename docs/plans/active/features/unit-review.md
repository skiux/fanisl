...
立于 2026-09-13。知识、base、frontend 三方已落地（2026-09-14）。**2026-09-17 取消写操作的管理员限制**（第 1 节第 6 条），当天落地（用户让 knowledge 席位直接改了 base 与 frontend 的文件，见第 6 节 3b / 3c）。
...
- 待确认入口放在**顶栏**（`frontend/src/shared/navigation/ReviewInbox.tsx`），不放知识库里：日常入口是标的页，
  答复要在每天都会经过的地方露头。没有待确认时整个入口不渲染。
- 直达链接 `#/knowledge?unit={id}&view=evidence&tab=review&review={id}`，落在那条核查上。
- 不分角色：待确认入口、提交、回复、关闭对所有登录用户开放。2026-09-14 的版本只给 admin，成员只读、
  界面显示"需要管理员权限"，2026-09-17 取消（第 1 节第 6 条）。账号菜单同时去掉了角色标签与「用户管理」。
...
| 3b | base | 三个写接口改为只要求登录；api.md 与测试同步（见 `base.md` Requests in） | member 可写；未登录仍 401 · **已完成**（2026-09-17） |
| 3c | frontend | 去掉知识站上按角色区分的入口、按钮与账号菜单项（见 `frontend.md` Requests in） | member 账号能看到并使用提交、回复、关闭，截图 · **已完成**（2026-09-17，夹具验收：成员身份跑完 e2e 两套视口） |
