...
| K4 | **验证**：daily_bars 价格层（90 符号 + 2 个 FRED 序列，yfinance/FRED，与语料屏价互验）+ scoring_overrides（77 条 success_def 机械化编译，质量核心）+ 评分器×5（含条件解析/守护条件/组合腿）+ scoreboard API + 前端联赛表与单元评分徽标；**首轮 60 时点评分 ✅ 2026-07-17**：Andy 命中率 42%（sign 类 10/25，p=0.212 不显著）、美投君 5 条 1 hit；日常=prices+scorers 两条幂等 CLI 按天跑 | 可验证 | ✅ |
